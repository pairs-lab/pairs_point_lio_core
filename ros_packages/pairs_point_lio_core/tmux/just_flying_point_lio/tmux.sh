...
PROJECT_NAME=just_flying_point_lio
...
'
  'Nimbro' 'waitForTime; rosrun pairs_uav_deployment run_nimbro.py ./config/network_config.yaml
'
  'HwApi' 'waitForTime; roslaunch pairs_uav_px4_api api.launch
'
  'Status' 'waitForHw; roslaunch pairs_uav_status status.launch
'
  'Core' 'waitForTime; roslaunch pairs_uav_core core.launch\
    platform_config:=`rospack find pairs_uav_deployment`/config/pairs_uav_system/$UAV_TYPE.yaml\
...
# Launch Livox + Point-LIO + Livox custom msg-type ROS republisher
  'LIO' 'waitForRos; roslaunch pairs_point_lio_core point_lio_livox_mid360.launch
'
