...
PRE_WINDOW='export UAV_NAME=uav1; export RUN_TYPE=simulation; export UAV_TYPE=x500; export SENSORS=ouster'
...
tmux send-keys -t "$P" "$SETUP; "'waitForGazebo; rosservice call /pairs_drone_spawner/spawn "1 $UAV_TYPE --enable-ground-truth --enable-ouster model:=OS0-64 horizontal_samples:=1024 use_gpu:=True --pos 0 0 0 0"' Enter
...
tmux send-keys -t "$P" "$SETUP; "'waitForHw; roslaunch pairs_point_lio_core point_lio_gazebo_os64.launch custom_config:=./custom_configs/point_lio_os64.yaml' Enter
