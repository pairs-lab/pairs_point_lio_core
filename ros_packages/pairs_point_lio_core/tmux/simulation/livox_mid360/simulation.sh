...
tmux send-keys -t "$P" "$SETUP; "'waitForRos; roslaunch pairs_uav_gazebo_simulation simulation.launch world_name:=forest gui:=true spawner_config:=./custom_configs/spawner_config.yaml' Enter
...
tmux send-keys -t "$P" "$SETUP; "'waitForGazebo; rosservice call /pairs_drone_spawner/spawn "1 x500_mid360 --enable-ground-truth --pos 0 0 0 0"' Enter
...
tmux send-keys -t "$P" "$SETUP; "'waitForHw; roslaunch pairs_uav_core core.launch platform_config:=`rospack find pairs_uav_gazebo_simulation`/config/pairs_uav_system/$UAV_TYPE.yaml custom_config:=./custom_configs/custom_config.yaml world_config:=./custom_configs/world_config.yaml network_config:=./custom_configs/network_config.yaml' Enter
tmux select-layout -t "$W_core" tiled

# ---------------- window: slam ----------------
read W_slam P <<< "$(tmux new-window -t "$SESSION_NAME" -n "slam" -P -F '#{window_id} #{pane_id}')"
tmux send-keys -t "$P" "$SETUP; "'waitForHw; roslaunch pairs_point_lio_core point_lio_gazebo_mid360.launch custom_config:=./custom_configs/point_lio_mid360.yaml' Enter
tmux select-layout -t "$W_slam" tiled

# ---------------- window: livox_tf ----------------
read W_livox_tf P <<< "$(tmux new-window -t "$SESSION_NAME" -n "livox_tf" -P -F '#{window_id} #{pane_id}')"
tmux send-keys -t "$P" "$SETUP; "'waitForHw; rosrun tf2_ros static_transform_publisher 0 0 0.2 0 0 0 $UAV_NAME/fcu $UAV_NAME/livox' Enter
tmux select-layout -t "$W_livox_tf" tiled
...
tmux send-keys -t "$P" "$SETUP; "'waitForControl; roslaunch pairs_aloam_core rviz.launch' Enter
